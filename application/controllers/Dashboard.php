...
public function __construct()
	...
	if ($del) 
		...
		redirect('Dashboard/changerm/use');	
	...
	public function changeAccStatus()
	{
		...
		if ($this->form_validation->run() == false) 
		{
			...
			redirect('Dashboard/changerm/acclt');
			return;
		}
	...
	public function changeLoanStatus()
	{
		...
		if ($this->form_validation->run() == false) 
		{
			...
			redirect('Dashboard/changerm/lonp');
			return;
		}
	...
	if (!$id_enc) 
		{
			show_404();
			return;
		} else{ $data['nav_file'] = "res/user/user_nav"; }
	...
	if ($saved) 
		{
			...
			redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/uset',"refresh");
			return;
		}
		else
		{
...
$saved = $this->Customer_model->quick_cash_ad($id);
	if ($saved) 
		{
			...
			redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/qkc',"refresh");
			return;
		}
		else
		{
	...
	$id = h_encrypt_decrypt($id_enc,'decrypt');
		...
		if ($this->form_validation->run() == false) 
		{
				...
				redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/wd');
			return;
		}
	...
	if ($saved) 
		{
			...
			//$this->index();
			redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/wd',"refresh");
			return;
		}
		else
		{
...
public function addUserBanks($id_enc)
	...
	$id = h_encrypt_decrypt($id_enc,'decrypt');
		...
		redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/bd',"refresh");
			return;
		}
		...
		public function contra()
	{
		...
		if ($this->form_validation->run() == false) 
		{
			...
			redirect('Dashboard/changerm/appv',"refresh");
			return;
		}
...
public function update_sign($id_enc)
	...
	if ($saved) 
		{
			...
			redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/sign',"refresh");
			return;
		}
		else
		{
	...
	$id = h_encrypt_decrypt($id_enc,'decrypt');
		...
		if ($this->form_validation->run() == false) 
		{
			...
			redirect('Dashboard/user_errors/'.h_encrypt_decrypt($id).'/lad',"refresh");
			return;
		}
