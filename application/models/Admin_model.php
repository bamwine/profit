...
FROM  tbl_loan_us   INNER JOIN tbl_usec ON tbl_loan_us.userIdl =tbl_usec.id WHERE  tbl_loan_us.userIdl = "'.$id.'"');
	...
	{
	$res = $this->db->query('SELECT * FROM tbl_loansc WHERE statusl= "pending"');
	...
	{
	$res = $this->db->query('SELECT tbl_loansc.*,tbl_usec.signature FROM tbl_loansc INNER JOIN tbl_usec ON tbl_loansc.userIdl = tbl_usec.id WHERE statusl= "approved"');
	...
	{
	$res = $this->db->query('SELECT * FROM tbl_loan_us WHERE statusl= "Transfered" and bal_to_pay!="0"');
