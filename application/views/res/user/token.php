...
<p>The token code has been sent to your email : <span style="color:#0066CC;font-weight:bold;"><?=$userdetails->email; ?></span></p>
<p>You have <span id="defaultCountdown"></span> minutes remaining to insert valid OTP. System will automatically redirect to 'Fund Transfer' page to initiate fund transfer again.</p>
                            <form class="form-material form-horizontal"  action="<?=base_url('Dashboard/Transferfud_ad/'.h_encrypt_decrypt($userdetails->id))?>" method="post" >
                                <div class="form-group">
                                    <label class="col-md-12" for="example-text">Transaction Authorization Code</span>
                                    </label>
                                    <div class="col-md-12">
       <input type="text" name="token" id="example-text" name="example-text" value="<?=$this->session->userdata('otp_token');?>" class="form-control"> </div>
                                </div>
